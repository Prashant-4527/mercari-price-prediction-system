...
def test_category_split_three_levels():
    df = pd.DataFrame([{
        "name": "Test Item", "item_condition_id": 1,
        "category_name": "Women/Bags/Shoulder Bag", "brand_name": "Chanel",
        "shipping": 0, "item_description": "test desc"
    }])
...
def test_missing_brand_defaults_to_no_brand():
    df = pd.DataFrame([{
        "name": "Test Item", "item_condition_id": 1,
        "category_name": "Women/Bags/Shoulder Bag", "brand_name": None,
        "shipping": 0, "item_description": "test desc"
    }])
    result = engineer_raw_features(df)
...
def test_missing_description_flagged_correctly():
    df = pd.DataFrame([{
        "name": "Test Item", "item_condition_id": 1,
        "category_name": "Women/Bags/Shoulder Bag", "brand_name": "Chanel",
        "shipping": 0, "item_description": None
    }])
    result = engineer_raw_features(df)
...
def test_condition_label_mapping():
    df = pd.DataFrame([{
        "name": "Test Item", "item_condition_id": 1,
        "category_name": "Electronics/Phones", "brand_name": "Apple",
        "shipping": 1, "item_description": "test"
    }])
    result = engineer_raw_features(df)
